...
const { blastType } = require('./blast-typing');
...
async function runPipeline(jobDir, emit, options = {}) {
  ...
  try {
    ...
    const [onlineResult, localResults, blastResults] = await Promise.all([
      ...
      // BLAST typing (NCBI) — sequential to respect rate limits
      (async () => {
        if (!genogroup) return null; // Skip BLAST if no genogroup specified
        const results = [];
        for (let i = 0; i < queries.length; i++) {
          const q = queries[i];
          emit('typing', `BLAST typing: ${i + 1}/${queries.length} — ${q.name}`);
          try {
            const result = await blastType(q.name, q.sequence, { genogroup });
            results.push(result);
          } catch (err) {
            emit('typing', `BLAST error for ${q.name}: ${err.message}`);
            results.push({
              name: q.name,
              length: q.sequence.length,
              blast: { error: err.message },
              noro: { capsid: '', polymerase: '' },
              rota: { gType: '', pType: '' },
            });
          }
        }
