...
/**
 * Extract genotype from local BLAST hit title
 */
function parseGenotypeFromHit(title) {
  // Title formats: 
  // - "GII.4_NC_029829..."
  // - "rva-vp4-P[8]..."
  // - "X86560_Sapovirus_GI.1"
  // - "NC_001943_Astrovirus_HAstV-1"
  // - "NC_001454_Adenovirus_HAdV-40"
  ...
  if (text.includes('noro') || text.includes('nov-')) {
    ...
    if (gMatch && pMatch) genotype = `${gMatch[0]}${pMatch[0]}`;
    else if (gMatch) genotype = gMatch[0];
    else if (pMatch) genotype = pMatch[0];
        ...
        for (const item of searchResults) {
          ...
          if (hits.length > 0) {
            ...
            for (const hit of hits.slice(0, 3)) {
              ...
              const parsed = parseGenotypeFromHit(hitTitle);
              if (parsed.genotype) {
