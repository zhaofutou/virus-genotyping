...
function extractOnlineByIndex(noroResults, rotaResults) {
  ...
  if (rotaResults) {
    for (const row of rotaResults) {
      ...
      const val2 = row['cluster/support_2'] || row['cluster/support subtype'] || '';
      let gGen = '', pGen = '';
...
/**
 * Build combined 1-72 summary with local + online columns.
 * Returns { master: [{index, local, online}], ... }
 */
function buildComparisonSummary(uploadedIndices, localResults, onlineNoroResults, onlineRotaResults) {
  ...
  for (let i = 1; i <= 72; i++) {
    ...
    master.push({ index: i, local: localResult, online: onlineResult });
  }
