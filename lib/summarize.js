...
function buildComparisonSummary(uploadedIndices, localResults, onlineNoroResults, onlineRotaResults, blastResults) {
  ...
  if (blastResults) {
    for (const r of blastResults) {
      ...
      const idx = parsed.index;
      const parts = [];
      if (r.virus && r.genotype) {
        const prefix = r.virus === 'norovirus' ? 'Noro' :
                       r.virus === 'rotavirus' ? 'Rota' :
                       r.virus === 'sapovirus' ? 'Sapo' :
                       r.virus === 'astrovirus' ? 'Astro' :
                       r.virus === 'adenovirus' ? 'Adeno' : r.virus;
        parts.push(`${prefix} ${r.genotype}`);
      } else {
      ...
      const val = parts.join(', ') || (r.blast?.error ? `error: ${r.blast.error}` : '');
      if (val) {
        if (!blastByIndex[idx]) {
          blastByIndex[idx] = val;
        } else {
          const existing = blastByIndex[idx];
          if (existing.startsWith('Rota ') && val.startsWith('Rota ')) {
            const extG = existing.match(/G\d+/i)?.[0] || val.match(/G\d+/i)?.[0] || '';
            const extP = existing.match(/P\[\d+\]/i)?.[0] || val.match(/P\[\d+\]/i)?.[0] || '';
            blastByIndex[idx] = `Rota ${extG}${extP}`;
          } else if (existing.startsWith('Noro ') && val.startsWith('Noro ')) {
            const extCapsid = existing.match(/G[IVX]+\.[\w\-]+/i)?.[0] || val.match(/G[IVX]+\.[\w\-]+/i)?.[0] || '';
            const extPol = existing.match(/P[\w\-]+/i)?.[0] || val.match(/P[\w\-]+/i)?.[0] || '';
            blastByIndex[idx] = `Noro ${extCapsid}[${extPol}]`;
          } else {
            const set = new Set([...existing.split(', '), ...val.split(', ')]);
            blastByIndex[idx] = [...set].join(', ');
          }
        }
      }
    }
